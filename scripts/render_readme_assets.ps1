...
$stdout = Join-Path $env:TEMP ("gtm-datalayer-standards-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("gtm-datalayer-standards-" + [guid]::NewGuid().ToString() + "-stderr.log")
$port = "5322"
$process = $null
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/event-contracts"; File = "02-event-contracts-proof.png"; Size = "1600,1420" },
        @{ Url = "http://127.0.0.1:$port/payload-lane"; File = "03-payload-lane-proof.png"; Size = "1600,1380" },
